  ...
  signup: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      try {
        ...
        // eslint-disable-next-line no-constant-condition -- This constant loop is fine...because worst case we only loop O(n) times.
        while (true) {
  ...
  update: protectedProcedure
    .input(
      z.object({
        styling: z
          .number()
          .min(0, 'Invalid styling option.')
          .max(len, 'Invalid styling option.'),
      }),
    )
    .mutation(async ({ input, ctx }) => {
      const { styling } = input;
